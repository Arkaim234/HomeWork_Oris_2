  ...
  return (
    <div className={styles['hotel-info-grid']}>
      ...
      <InfoRow label="Основная информация">
        ...
        {hotel.buildingInfo && (
          <div dangerouslySetInnerHTML={{ __html: hotel.buildingInfo }} />
        )}
      </InfoRow>
      ...
      {hotel.contacts && hotel.contacts.length > 0 && (
        <InfoRow label="Контакты">
          <ul className={styles['hotel-contacts']}>
            {hotel.contacts.map((c, i) => <li key={i}>{c}</li>)}
          </ul>
        </InfoRow>
      )}
  ...
  return (
    <div className={styles['hotel-info-grid']}>
      <InfoRow label="В номере">
        <InfoList items={hotel.inRoomServices} />
      </InfoRow>
        ...
        {hotel.roomTypes && hotel.roomTypes.length > 0 ? (
          <ul className={styles.plainList}>
            {hotel.roomTypes.map(room => (
              <li key={room.id}>
                <strong>{room.name}</strong> — вид: {room.view},&nbsp;
                кровати: {room.bedConfiguration},&nbsp;
                макс. {room.maxOccupancy} чел.
  ...
  return (
    <div className={styles['hotel-info-grid']}>
        ...
        {hotel.mealPlans && hotel.mealPlans.length > 0 ? (
          <ul className={styles.plainList}>
            {hotel.mealPlans.map(mp => (
              <li key={mp.code}><strong>{mp.code}</strong> — {mp.description}</li>
            ))}
          </ul>
        ) : (
          <p>Нет данных.</p>
      ...
      <InfoRow label="Для детей">
        <InfoList items={hotel.childServices} />
      </InfoRow>

      <InfoRow label="Развлечения и спорт">
        {hotel.freeEntertainment && hotel.freeEntertainment.length > 0 && (
          <>
            <p><strong>Бесплатно:</strong></p>
            <InfoList items={hotel.freeEntertainment} />
          </>
        )}
        {hotel.paidEntertainment && hotel.paidEntertainment.length > 0 && (
          <>
      ...
      <InfoRow label="Услуги на территории">
        <InfoList items={hotel.onSiteServices} />
      </InfoRow>
    </div>
  )
}
    ...
    return (
      <div className="cmn-l-content cmn-l-content_main">
        <div className="cmn-l-block-wrap">
          ...
          <p>Проверьте адрес или вернитесь на <Link to="/turkey/">главную</Link>.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="cmn-l-content cmn-l-content_main">
      ...
      <div className="cmn-l-breadcrumbs">
        <div className="cmn-l-breadcrumbs__items">
          <Link className="cmn-l-breadcrumbs__item" to="/turkey/">Главная</Link> »&nbsp;
          <Link className="cmn-l-breadcrumbs__item" to="/turkey/">Турция</Link> »&nbsp;
      ...
      <div className="cmn-l-block-wrap">
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`${styles['pages-main-nav__item']} ${
                activeTab === tab.key ? styles['pages-main-nav__item_active'] : ''
              }`}
        ...
        {/* Галерея — та же что на главной, с пропсами */}
        <HotelGallery
